...
mapNextAddr (VarMap a b) = VarMap a (b+1)
nextAddr (VarMap _ a) = a + 1
getAddr (VarMap _ a) = DirAddr a
...
--nextAddr map =  DirAddr $ (countMap map) + maxThread
...
genVarDec map varName expr = (instrs, newVarMap)
  where instrs = genExpr map expr
        map = storeVar (VarMem varName (getAddr map)) varMap
...
--        (scopeInstrs, updatedMap) = genInstrs' (VarMap map []) stmts []
--        map2 = storeVar (MapMem updatedMap) map
